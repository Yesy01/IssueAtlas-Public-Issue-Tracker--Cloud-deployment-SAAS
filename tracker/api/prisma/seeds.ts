...
async function main() {
  ...
  const issue1 = await prisma.issue.create({
    data: {
      ...
      description:
        "Deep pothole on the main road near the central market. Causes traffic jams and is unsafe for bikes.",
      status: "new",
      type: "pothole",
      lat: 19.0760,
      lon: 72.8777,
  ...
  const issue2 = await prisma.issue.create({
    data: {
      ...
      description:
        "Streetlight has been off for over a week. Lane is very dark at night and unsafe for pedestrians.",
      status: "triaged",
      type: "streetlight",
      lat: 12.9716,
      lon: 77.5946,
  ...
  const issue3 = await prisma.issue.create({
    data: {
      ...
      description:
        "Drain is blocked and stagnant water is accumulating. Mosquitoes breeding in the area.",
      status: "in_progress",
      type: "drainage",
      lat: 13.0827,
      lon: 80.2707,
  ...
  const issue4 = await prisma.issue.create({
    data: {
      ...
      description:
        "Several slabs are broken, children walking to school are at risk of tripping.",
      status: "resolved",
      type: "other",
      lat: 28.7041,
      lon: 77.1025,
  ...
  const issue5 = await prisma.issue.create({
    data: {
      ...
      description:
        "Garbage bin has not been cleared for days, trash spilling onto street.",
      status: "new",
      type: "other",
      lat: 22.5726,
      lon: 88.3639,
